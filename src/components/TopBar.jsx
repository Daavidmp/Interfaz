    ...
    <header className="sticky top-0 z-50 flex items-center justify-between p-4 bg-gradient-to-r from-pink-700 to-purple-900 rounded-b-2xl shadow-2xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-sm text-white/80">Grupo: {group?.name || 'Sin grupo'}</div>
        </div>
      </div>
      <div className="flex items-center gap-4">
        <div className="flex flex-col items-end">
            <div className="text-white font-semibold text-lg">{user?.username}</div>
            <div className="text-xs text-pink-300">Balance: ₿{user?.balance || '0'} | ❤️{user?.lives || '0'}</div>
        </div>
        <button onClick={handleLogout} className="px-4 py-2 rounded-full bg-white/20 hover:bg-white/30 transition duration-300 font-semibold shadow-md">
            Salir
        </button>
      </div>
    </header>
